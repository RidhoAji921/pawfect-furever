...
<body class="bg-gradient-to-t from-[#F8EDE3] from-0% via-[#F6B171] via-50% to-[#F7D1AE] to-80% flex flex-col justify-center items-center py-5 lg:px-32 px-5 w-full gap-10 bg-no-repeat bg-cover min-h-screen">
    <div class="flex flex-col w-fit justify-center items-center h-full">
        ...
        <div class="flex flex-col gap-10 p-10 rounded-[20px] bg-Orange/20 items-center w-full drop-shadow-[4px_8px_4px_rgba(0,0,0,0.25)]">
            ...
            <form method="POST" action="{{ route('password.update') }}" class="flex flex-col gap-5 w-full">
                @csrf
                <input type="hidden" name="token" value="{{ $token }}">
                <div class="flex flex-col gap-5 w-full">
                    <label for="email" class="text-textTitle font-Quicksand lg:text-Header4 text-Large font-bold">Password Baru :</label>
                    <input type="email" name="email" id="email" placeholder="Masukan email anda ... " class="text-white opacity-text-60 font-Quicksand lg:text-Large text-Base font-medium bg-Orange/70 px-4 py-2 rounded-[16px] w-full" ></input>
                    @error('email')
                        <p class="text-red-600">{{ $message }}</p>
                    @enderror
                    <label for="password" class="text-textTitle font-Quicksand lg:text-Header4 text-Large font-bold">Password Baru :</label>
                    <input type="password" name="password" id="password" placeholder="Masukan password anda ... " class="text-white opacity-text-60 font-Quicksand lg:text-Large text-Base font-medium bg-Orange/70 px-4 py-2 rounded-[16px] w-full" ></input>
                    @error('password')
                        <p class="text-red-600">{{ $message }}</p>
                    @enderror
                    <label for="password_confirmation" class="text-textTitle font-Quicksand lg:text-Header4 text-Large font-bold">Konfirmasi Password Baru :</label>
                    <input type="password" name="password_confirmation" id="password_confirmation" placeholder="Konfirmasi password anda ... " class="text-white opacity-text-60 font-Quicksand lg:text-Large text-Base font-medium bg-Orange/70 px-4 py-2 rounded-[16px] w-full" ></input>
                    @error('password_confirmation')
                        <p class="text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <button type="submit" class="flex justify-center items-center w-full gap-2 px-10 lg:py-4 py-3 bg-Orange/10 rounded-2xl text-white font-Quicksand lg:text-Header4 text-Large font-bold">Reset Password
