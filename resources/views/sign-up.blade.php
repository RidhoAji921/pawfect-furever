...
<body class="bg-gradient-to-t from-[#F8EDE3] from-0% via-[#F6B171] via-50% to-[#F7D1AE] to-80% flex flex-col justify-center items-center py-5 px-32 gap-10">
    <div class="flex flex-col w-fit justify-center items-center">
        ...
        <div class="flex flex-col gap-10 p-10 rounded-[20px] bg-Orange/20 items-center w-full drop-shadow-[4px_8px_4px_rgba(0,0,0,0.25)]">
            <h1 class="text-textTitle font-Quicksand text-Header2 font-bold">Daftar Akun</h1>
            <form method="POST" action="{{ route('signup') }}" class="flex flex-col gap-5 w-full">
                ...
                <div class="flex flex-col gap-3 w-full">
                    ...
                    <input name="email" type="email" value="{{ old('email') }}" placeholder="Masukan email anda ..." class="text-white opacity-text-60 font-Quicksand text-Large font-medium bg-Orange/70 px-4 py-2 rounded-[16px] w-full"></input>
                    @error('email')
                    <div class="text-[#FF181C] font-Quicksand text-base font-medium">{{ $message }}</div>
                    @enderror
                    <p class="{{--text-[#FF181C]--}}text-[#2a2a2a] font-Quicksand text-base font-medium">Gunakan email aktif untuk informasi lebih lanjut!</p>
                </div>
                <div class="flex flex-col gap-3 w-full">
                    <label for="phone" class="text-textTitle font-Quicksand text-Header4 font-bold">No Hp (opsional) :</label>
                    <input name="phone" type="text" value="{{ old('phone') }}" placeholder="Masukan no hp anda ... " class="text-white opacity-text-60 font-Quicksand text-Large font-medium bg-Orange/70 px-4 py-2 rounded-[16px] w-full" ></input>
                    @error('phone')
                    <div class="text-[#FF181C] font-Quicksand text-base font-medium">{{ $message }}</div>
                    @enderror
                    <div>
                        <p class="text-[#2a2a2a] font-Quicksand text-base font-medium">Gunakan no hp untuk pemberitahuan via WhatsApp</p>
                        <p class="text-[#2a2a2a] font-Quicksand text-base font-medium">Gunakan format 08XXXXXXXXXX</p>
                        <p class="text-[#2a2a2a] font-Quicksand text-base font-medium">Kosongkan untuk pemberitahuan via email</p>
                    </div>
                </div>
                <div class="flex flex-col gap-3 w-full">
                    <label for="name" class="text-textTitle font-Quicksand text-Header4 font-bold">Nama :</label>
                    <input name="name" type="text" value="{{ old('name') }}" placeholder="Masukan nama anda " class="text-white opacity-text-60 font-Quicksand text-Large font-medium bg-Orange/70 px-4 py-2 rounded-[16px] w-full" ></input>
                    @error('name')
                    <div class="text-[#FF181C] font-Quicksand text-base font-medium">{{ $message }}</div>
                    @enderror
                </div>
                <div class="flex flex-col gap-3 w-full">
                    ...
                    <div class="text-[#FF181C] font-Quicksand text-base font-medium">{{ $message }}</div>
                    @enderror
                    <label for="password_confirmation" class="text-textTitle font-Quicksand text-Header4 font-bold">Konfirmasi Password :</label>
                    <input name="password_confirmation" type="password" placeholder="Masukan password anda ... " class="text-white opacity-text-60 font-Quicksand text-Large font-medium bg-Orange/70 px-4 py-2 rounded-[16px] w-full" ></input>
                </div>
                <button type="submit" class="flex justify-center items-center w-full gap-2 px-10 py-4 bg-Orange/10 rounded-2xl text-white font-Quicksand text-Header4 font-bold">Register
            ...
            <p class="text-white font-Quicksand text-Header4 font-semibold">Sudah punya akun?<a href="/login"><span class="text-textTitle font-Quicksand text-Header4 font-semibold">Login Sekarang</span></a></p>
        </div>
    </div>
